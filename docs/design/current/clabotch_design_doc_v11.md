...
### 10.4 Hook シェル設定（v11最終版）

> **v9〜v11 の変更点（設計書 §10.4）：**  
> v10: `json_escape()` を `jq -R .` に変更 / `jq` 必須化 / grep fallback 撤廃。  
> v11: `resolve_session_id` が空文字列を返すよう変更 / 全スクリプトに session_id 欠損時 drop-and-log guard 追加。
...
| v11 | `resolve_session_id` が空文字列を返すよう変更 / 全フックスクリプトに session_id 欠損時 drop-and-log guard 追加（exit 1・非ブロッキング）/ `v1〜v8 変更履歴` 見出しを `v1〜v11 変更履歴` に修正 / 成果物一覧に `clabotch_post_tool_failure.sh` を追加 |
...
| ファイル | 内容 |
|----------|------|
| `clabotch_design_doc_v11.md` | 本仕様書（v1〜v11統合、実装開始可能） |
...
| `~/.claude/hooks/clabotch_post_tool_failure.sh` | PostToolUseFailure hook |
| `~/.claude/hooks/clabotch_stop.sh` | Stop hook |

---

*Clabotch — MIT License — 2026 Nakata*  
*v11: 2026-03-10 — session_id 欠損時 drop-and-log・changelog 見出し修正*
