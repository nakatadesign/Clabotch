...
### 10.4 Hook シェル設定（v9最終版）

> **v8 → v9 の差分（設計書 §10.4 のみ変更）：**  
> フックのデータ受け取り方が **環境変数ではなく stdin JSON** であることが確認された。  
> `$SESSION_ID` / `$TOOL_NAME` / `$EXIT_CODE` / `$TOOL_DURATION_MS` は存在しない。  
> 加えて `PostToolUse` は成功時のみ発火するため、エラー検知は `PostToolUseFailure` を別途フックする。
...
```bash
#!/usr/bin/env bash
# Clabotch hook helper — v9
# データは stdin JSON で受け取る。環境変数への依存は最小限にする。
...
# 呼び元スクリプトの先頭で: HOOK_JSON=$(read_stdin)
read_stdin() {
  cat
}

# session_id を stdin JSON と env var から解決する
# 引数 $1: 事前に読んだ HOOK_JSON
resolve_session_id() {
  local json="$1"
  local sid
  sid=$(echo "$json" | jq -r '.session_id // empty' 2>/dev/null)
  # jq がない環境向け grep fallback
  if [[ -z "$sid" ]]; then
    sid=$(echo "$json" | grep -o '"session_id":"[^"]*"' | cut -d'"' -f4)
  fi
  # v2.1.9+ の環境変数 fallback
  echo "${sid:-${CLAUDE_SESSION_ID:-unknown}}"
}

# tool_name を stdin JSON から解決する
resolve_tool_name() {
  local json="$1"
  local name
  name=$(echo "$json" | jq -r '.tool_name // "unknown"' 2>/dev/null)
  if [[ -z "$name" ]]; then
    name=$(echo "$json" | grep -o '"tool_name":"[^"]*"' | cut -d'"' -f4)
  fi
  echo "${name:-unknown}"
}
...
TOOL_NAME=$(resolve_tool_name "$HOOK_JSON")
...
# ③ tool_start（NDJSON: \n 末尾必須）
TOOL_ESCAPED=$(json_escape "$TOOL_NAME")
...
# ② PostToolUse は成功時のみ発火するため is_error=false 固定
TOOL_ESCAPED=$(json_escape "$TOOL_NAME")
...
> **jq の有無について:**  
> `resolve_session_id` / `resolve_tool_name` は `jq` があれば使い、なければ `grep` フォールバックで動く。  
> 確実な運用のため `brew install jq` を推奨する。
...
| ファイル | 内容 |
|----------|------|
| `clabotch_design_doc_v9.md` | 本仕様書（v1〜v9統合、実装開始可能） |
...
*v9: 2026-03-10 — Hook stdin JSON 修正・実装開始可能版*
