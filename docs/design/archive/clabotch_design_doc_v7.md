...
final class GazeController {
    ...
    private func update() {
        ...
        if let target = center, let origin = statusItemCenterProvider?() {
            mode = .tracking
...
> - v7: sleeping タイマー再始動（`transition(.idle)` 内で `startSleepTimer()`）
> - v7: 重複 `session_start` の冪等化（`isOwned` を `session == nil` のみ受理）

```swift
// MARK: - StateMachine.swift（v7最終版）
//
// 設計の核心:
//   isOwned()         副作用ゼロで ownership を先に判定
//   cancelSleepTimer  タイマーキャンセルのみ担当（Step 2 で呼ぶ）
//   startSleepTimer   transition(.idle) 内でのみ呼ぶ（再始動を確実にする）
...
final class StateMachine {
    ...
    func handle(event: ClabotchEvent) {
        ...
        cancelSleepTimer()   // キャンセルのみ。タイマー開始は transition(.idle) が担う

        // ── Step 3: 状態遷移（ownership 確定済み）─────────────────────
        switch event {

        case .sessionStart(let sessionID):
            // isOwned が session == nil のみ受理するため、ここに来たときは session == nil が保証される
        ...
        case .sessionDone(_, let elapsedMs):
            ...
            // 4秒後に transition(.idle) → startSleepTimer() が自動的に呼ばれる
    ...
    private func transition(to phase: MascotPhase) {
        ...
        // v7: .idle に遷移した瞬間かつ session がない場合に sleeping タイマーを再始動。
        // 遷移経路（scheduleAutoTransition / cancelSleepTimer内部）に関わらず確実に再始動する。
        if case .idle = phase, session == nil {
            startSleepTimer()
        }
    ...
    // v7: タイマーのキャンセルと sleeping 解除のみを担当。
    // タイマー開始ロジックは transition(to: .idle) 内の startSleepTimer() が担う。
    private func cancelSleepTimer() {
        sleepTimer?.invalidate()
        sleepTimer = nil
        if displayPhase == .sleeping {
            // sleeping 中に owned event が来た → 適切なフェーズに戻す
            // session == nil なら transition(.idle) → startSleepTimer() が発火する
            transition(to: session != nil ? .thinking : .idle)
        }
    }

    // sleeping タイマーを（再）始動する。
    // 呼び出し前提: session == nil かつ displayPhase == .idle
...
| v7 | sleeping タイマー再始動（`transition(.idle)` 内で `startSleepTimer()`） / 重複 session_start 冪等化 / GazeController `startPolling()` 追加 |
...
| ファイル | 内容 |
|----------|------|
| `clabotch_design_doc_v7.md` | 本仕様書（v1〜v7統合、実装開始可能） |
...
*v7: 2026-03-10 — v2〜v7 全パッチ統合・実装開始可能版*
