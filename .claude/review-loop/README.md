...
`run_judge.sh` は `SUPERVISOR.md` を前提に、reviewer 結果の優先順位付けと停止条件の判定を行います。
...
4. Claude は各 tick で `status.sh` を見て、必要なら `record_claude_round.sh` → `run_reviewer.sh` → `run_judge.sh` を実行する
