...
このリポジトリでは `.claude/totonoe/` が既に同梱されています（embedded runtime）。`setup.sh` の実行は不要です。

> **Note**: `setup.sh` は upstream テンプレートリポジトリから新規リポジトリへ導入する際に使うものです。この repo には適用されません。
...
3. `.claude/agents/manager.md` の dispatch テーブルに行を追加する
